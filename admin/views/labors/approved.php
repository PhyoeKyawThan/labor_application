...
$applications = $lbModel->getApplicationByStatus('Approved');
if (isset($_GET['s'])) {
    $applications = $lbModel->search($_GET['s']);
}
?>
<h1>Approved Laborers List</h1>
<p style="color: red"><?= $delete_msg ?? '' ?></p>
<div class="search-form">
    <form action="" method="GET">
        <input type="hidden" name="vr" value="labors">
        <input type="hidden" name="stus" value="approved">
    ...
    <tbody>
        ...
        foreach ($applications as $laborer):
            ...
            <tr>
                ...
                <td class="actions">
                    <a href="?vr=labors&stus=approved&lid=<?= $laborer['id'] ?>">View</a>
                    <a href="?vr=labors&did=<?= $laborer['id'] ?>&stus=approved"
