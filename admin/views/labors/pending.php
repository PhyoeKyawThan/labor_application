...
$applications = $lbModel->getApplicationByStatus('Pending');
if (isset($_GET['s'])) {
    $applications = $lbModel->search($_GET['s']);
}
?>
<h1>Pending Laborers List</h1>
    ...
    <tbody>
        ...
        foreach ($applications as $laborer):
            ...
            <tr>
                ...
                <td class="actions">
                    <a href="?vr=labors&stus=pending&lid=<?= $laborer['id'] ?>">View</a>
                    <a href="?vr=labors&did=<?= $laborer['id'] ?>&stus=pending"
