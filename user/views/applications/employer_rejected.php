...
$form  = $model->readFormById($user_id);
// $req_form = $model->readDetails($formform_id);
$occupation =json_decode( $form['occupation'], true)[0];
?>
...
<form action="" method="post" class="modern-form">
    ...
    <div class="form-row">
        ...
        <div class="form-group">
            ...
            <input type="text" name="position" value="<?= $form['position'] ?>" id="position">
        </div>
    </div>

    <div class="form-group">
        <label for="depart-address">Department Address</label>
        <textarea name="department_address" id="depart-address"><?= $form['department_address'] ?></textarea>
    </div>
    ...
    <div class="form-row">
        <div class="form-group">
            ...
            <input type="text" name="letter_no" id="letter_no" value="<?= $form['department_address'] ?? '-' ?>">
        </div>
    </div>

    <div class="form-group">
        <label for="report-receiver">Name and position of the person to whom the workers must report.</label>
        <textarea name="report_receiver" id="report-receiver"><?= $form['report_receiver'] ?></textarea>
    </div>
                ...
                <div class="form-group">
                    ...
                    <select name="occupation[0][occupation]" id="occupation-0">
                        <?php foreach (array_keys($position_codes['employee_codes']) as $position):
                            ?>
                            <option value="<?= $position ?>" <?= $occupation['occupation'] == $position ? 'selected' : '' ?>><?= $position ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <label for="male-0">Male</label>
                    <input type="number" name="occupation[0][male]" value="<?= $occupation['male'] ?>" id="male-0">

                    <label for="female-0">Female</label>
                    <input type="number" name="occupation[0][female]" value="<?= $occupation['female'] ?? 0 ?>" id="female-0">
                </div>
                <div class="form-group">
                    ...
                    <select name="occupation[0][qualification]" id="qualification-0">
                        <?php foreach ($educationLevels as $edu_level):
                            ?>
                            <option value="<?= $edu_level ?>" <?= $occupation['qualification'] == $edu_level ? 'selected' : '' ?>><?= $edu_level ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
